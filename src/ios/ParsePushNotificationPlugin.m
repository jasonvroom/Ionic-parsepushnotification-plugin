...
ParsePushNotificationPlugin *ppp;
...
- (void)setUp: (CDVInvokedUrlCommand*)command {
    ...
    ppp = [[ParsePushNotificationPlugin alloc] init];
    ppp = self;
...
- (void)swizzled_application:(UIApplication *)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)newDeviceToken
{
    ...
    [currentInstallation saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        ...
        if (succeeded) {
            CDVPluginResult* pr = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:@"onRegisterAsPushNotificationClientSucceeded"];
            [pr setKeepCallbackAsBool:YES];
            [ppp.commandDelegate sendPluginResult:pr callbackId:ppp.callbackIdKeepCallback];
            NSLog(@"onRegisterAsPushNotificationClientSucceeded");
            
        } else {
            // There was a problem, check error.description
            CDVPluginResult* pr = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"onRegisterAsPushNotificationClientFailed"];
            [pr setKeepCallbackAsBool:YES];
            [ppp.commandDelegate sendPluginResult:pr callbackId:ppp.callbackIdKeepCallback];
        }
    }];
